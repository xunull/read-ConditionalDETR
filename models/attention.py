...
因为在cross attention时，query和key的维度与value的维度不相同

移除了query, Key, value的投影
在torch的实现中
...
class MultiheadAttention(Module):
    r"""
    Encoder的Attention还是用的torch的实现的
    Decoder的两个Attention用的都是这一个
    ...
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False, kdim=None,
        ...
        # Self Attention是相同的 Cross Attention是不同的
    ...
    Shape:
        ...
        if not self._qkv_same_embed_dim:
            ...
            return multi_head_attention_forward(
                ...
                attn_mask=attn_mask,
                # 下面四个是下面那种调用没有的, 不过这四个值在下面的方法中没有被使用
...
def multi_head_attention_forward(query: Tensor,
    ...
    if k is not None:
        k = k.contiguous().view(-1, bsz * num_heads, head_dim).transpose(0, 1) # [bs*8,hw,64]
    if v is not None:
        v = v.contiguous().view(-1, bsz * num_heads, v_head_dim).transpose(0, 1) # [bs*8,hw,32]
    ...
    # qk和v的矩阵乘法 [bs*8,300,hw] * [bs*8,hw,32] -> [bs*8,300,32]
    attn_output = torch.bmm(attn_output_weights, v)
    assert list(attn_output.size()) == [bsz * num_heads, tgt_len, v_head_dim]
    attn_output = attn_output.transpose(0, 1).contiguous().view(tgt_len, bsz, out_dim) # [300,bs,256]
    attn_output = linear(attn_output, out_proj_weight, out_proj_bias)

    if need_weights:
        # average attention weights over heads [bs,8,300,hw]
        attn_output_weights = attn_output_weights.view(bsz, num_heads, tgt_len, src_len)
